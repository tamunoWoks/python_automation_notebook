...
**Note:** The Windows Path objects have a `drive` attribute, but macOS and Linux `Path` objects don’t. The `drive` attribute doesn’t include the first backslash.  
...
p.parts[0:2] # ('C:\\', 'Users')
```
**Note:** Even though the string used in the `Path()` call contains forward slashes, parts uses an anchor on Windows that has the appropriate backslash: `'C:\\'` (or `r'C:\'` as a raw string with the backslash unescaped).
