...
You can also pass the method a string, such as 'hello' or '?'. For example, if the cursor isn’t currently in a text field, pressing the home and end keys will scroll the browser to the top and bottom of the page, respectively. Enter the following  and notice how the send_keys() calls scroll the page:
```python
from selenium import webdriver
from selenium.webdriver.common.by import By
