...
### Exploring APIs:
Other websites, such as *https://weather.gov* and *https://weatherapi.com*, provide their own free weather APIs. Every API works differently, but they’re often accessed as requests over HTTPS, in which case you can use the Requests library and return responses formatted as JSON or XML text. However, someone may have created a third-party Python package to make using 
these APIs easier, with Python functions that handle accessing the endpoints and parsing the response for you. You can find these packages on *https://pypi.org*; read the package documentation to learn about their use.
